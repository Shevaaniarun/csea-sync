...
const Shuffle: React.FC<ShuffleProps> = ({
  ...
  useGSAP(
    () => {
      ...
      const removeHover = () => {
        if (hoverHandlerRef.current && ref.current) {
          ref.current.removeEventListener('mouseenter', hoverHandlerRef.current as unknown as EventListener);
      ...
      const teardown = () => {
        ...
        if (wrappersRef.current.length) {
          wrappersRef.current.forEach((wrap) => {
            ...
            if (orig && wrap.parentNode) (wrap.parentNode as HTMLElement).replaceChild(orig, wrap);
          });
          wrappersRef.current = [] as unknown as HTMLSpanElement[];
        }
        try {
      ...
      const build = () => {
        ...
        chars.forEach((ch) => {
          ...
          wrappersRef.current.push(wrap as unknown as HTMLSpanElement);
        });
      };
      ...
      const play = () => {
        ...
        const tl = gsap.timeline({
          ...
          onComplete: () => {
            ...
            if (!loop) {
              cleanupToStill();
              if (colorTo) gsap.set(strips, { color: colorTo as string });
              onShuffleComplete?.();
              armHover();
            }
          },
        ...
        const addTween = (targets: HTMLElement[], at: number) => {
          ...
          if (colorFrom && colorTo) {
            tl.to(targets, { color: colorTo as string, duration, ease }, at);
          }
        };
        ...
        if (animationMode === 'evenodd') {
          ...
          if (odd.length) addTween(odd as HTMLElement[], 0);
          if (even.length) addTween(even as HTMLElement[], evenStart);
        } else {
      ...
      const armHover = () => {
        ...
        hoverHandlerRef.current = handler as unknown as (e: Event) => void;
        ref.current.addEventListener('mouseenter', handler as unknown as EventListener);
      };
    ...
    },
    {
      ...
      scope: ref as unknown as React.MutableRefObject<Element>,
    },
  );

  const commonStyle = { textAlign, ...style } as CSSProperties;
  const classes = `shuffle-parent ${ready ? 'is-ready' : ''} ${className}`;
  const Tag = (tag || 'p') as keyof JSX.IntrinsicElements;
  return React.createElement(Tag, { ref: ref as React.Ref<any>, className: classes, style: commonStyle }, text);
};
